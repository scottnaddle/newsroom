...
async function validateGhostPost(postId, jwt) {
  const curlCmd = `curl -s -X GET \
    "https://newsroom.ubion.global/ghost/api/admin/posts/${postId}/?formats=html" \
  ...
  try {
    ...
    const publishedArticle = {
      ...
      publish_result: {
        ...
        ghost_draft_url: `https://newsroom.ubion.global/ghost/#/editor/post/${ghostPost.id}`,
        status: 'published',
      ...
      audit_log: [
        ...article.audit_log,
        {
          ...
          note: `Ghost draft URL: https://newsroom.ubion.global/ghost/#/editor/post/${ghostPost.id}`
        }
      ]
    };
