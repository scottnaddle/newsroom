...
console.log('🎨 Draft 기사: 이미지 삭제 → Unsplash 재할당');
console.log('');

// Step 1: Draft 기사 조회
const getOptions = {
  hostname: GHOST_URL,
  path: '/ghost/api/admin/posts/?limit=100&filter=status:draft',
  method: 'GET',
  headers: {
...
const getReq = https.request(getOptions, (res) => {
  ...
  res.on('end', async () => {
    try {
      ...
      const draftPosts = response.posts;
      
      console.log(`Step 1️⃣ : 이미지 삭제 (${draftPosts.length}개)`);
      console.log('');
      ...
      console.log(`✅ ${deleted}개 이미지 삭제됨`);
      console.log('');
      console.log(`Step 2️⃣ : Unsplash 이미지 할당 (${draftPosts.length}개)`);
      console.log('');
      
      // Step 2: 모든 기사에 Unsplash 이미지 할당
      let assigned = 0;
      for (let i = 0; i < draftPosts.length; i++) {
        const post = draftPosts[i];
        const imageUrl = validImages[i % validImages.length];
        
        console.log(`${i + 1}/${draftPosts.length}. "${post.title.substring(0, 40)}..."`);
      ...
      console.log(`✅ 완료:`);
      console.log(`   ✅ ${deleted}개 이미지 삭제됨`);
      console.log(`   ✅ ${assigned}/${draftPosts.length}개 이미지 할당됨`);
      console.log('');
      console.log('🌐 웹사이트 새로고침 (Ctrl+Shift+R) 하면 모든 Draft 기사가 새로운 Unsplash 이미지를 가질 것입니다!');
