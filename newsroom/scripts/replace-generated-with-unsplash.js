...
console.log('🔄 AI 생성 이미지 → Unsplash 이미지로 변경');
console.log('');
...
const getReq = https.request(getOptions, (res) => {
  ...
  res.on('end', async () => {
    try {
      ...
      const generatedImages = response.posts.filter(p => {
        if (!p.feature_image) return false;
        return p.feature_image.includes('og-card') || 
               p.feature_image.includes('insight.ubion.global/content/images/');
      });

      console.log(`변경 대상: ${generatedImages.length}개 기사`);
      console.log('');
      
      let updated = 0;
      
      for (let i = 0; i < generatedImages.length; i++) {
        const post = generatedImages[i];
        
        // Unsplash 이미지 순환 배치
        const imageUrl = UNSPLASH_IMAGES[i % UNSPLASH_IMAGES.length];
        
        console.log(`${i + 1}/${generatedImages.length}. "${post.title.substring(0, 45)}..."`);
      ...
      console.log('');
      console.log(`✅ 완료: ${updated}/${generatedImages.length}개 기사 변경됨`);
      console.log('');
      console.log('🌐 웹사이트 새로고침 (Ctrl+Shift+R) 하면 모든 기사가 Unsplash 이미지로 표시될 것입니다!');
