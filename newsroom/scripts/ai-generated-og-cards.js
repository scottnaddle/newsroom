...
const categoryColors = {
  'education': '#0891b2',      // 교육 - 사이안
  'policy': '#4338ca',          // 정책 - 인디고
  'industry': '#d97706',         // 산업 - 앰버
  'research': '#059669',         // 연구 - 에메랄드
  'opinion': '#7c3aed',          // 의견 - 바이올렛
  'data': '#0284c7',             // 데이터 - 스카이
  'default': '#1f2937'           // 기본 - 그레이
};
  ...
  res.on('end', async () => {
    try {
      ...
      for (let i = 0; i < response.posts.length; i++) {
        ...
        console.log(`${i + 1}/${response.posts.length}. [${category.toUpperCase()}]`);
        ...
        // Ghost에 업로드
        const imageUrl = await uploadToGhost(imageBuffer, `og-card-${post.id}.png`);
      ...
      console.log(`✅ 완료: ${processed}/${response.posts.length}개 기사`);
      console.log('');
      console.log('🌐 웹사이트 새로고침 (Ctrl+Shift+R) 하면 AI 생성 OG 카드가 모두 보입니다!');
