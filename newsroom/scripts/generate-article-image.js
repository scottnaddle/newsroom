#!/usr/bin/env node
/**
 * === 뉴스룸 AI 기사 이미지 생성기 (v5.2 — FLUX 2 Turbo + BananaX + 문자억제) ===
 * FLUX 2 Turbo (FAL AI) → Ghost CMS 업로드
...
const GHOST_URL = loadEnv('GHOST_URL') || 'https://newsroom.ubion.global';
const GHOST_ADMIN_API = loadEnv('GHOST_ADMIN_API_KEY');

if (!FAL_KEY) { console.error('❌ FAL_KEY not found in .env'); process.exit(1); }
if (!GHOST_ADMIN_API) { console.error('❌ GHOST_ADMIN_API_KEY not found in .env'); process.exit(1); }
...
const COUNTRY_PATTERNS = [
  ...
  { match: /(?:프랑스|France|파리)/i, country: 'France', region: '프랑스', ethnic: 'French', setting: 'French classroom' },
  { match: /(?:독일|Germany|베를린)/i, country: 'Germany', region: '독일', ethnic: 'German', setting: 'German classroom' },
  { match: /(?:핀란드|Finland|헬싱키)/i, country: 'Finland', region: '핀란드', ethnic: 'Finnish', setting: 'Finnish classroom' },
...
const COUNTRY_CONTEXT = {
  'South Korea': { ethnic: 'Korean', style: 'Korean educational setting with school uniforms, modern smartboards, technology-integrated classrooms.' },
  ...
  'Japan': { ethnic: 'Japanese', style: 'Japanese educational setting with clean organized learning environment.' },
  'United Kingdom': { ethnic: 'multi-ethnic British', style: 'UK educational setting with blend of historic architecture and modern technology.' },
  'Africa': { ethnic: 'African (diverse ethnicities)', style: 'warm African educational setting with natural light and vibrant community.' },
...
// ─── 🍌 BananaX 301 스타일 팔레트 (22가지, v5.1 문자억제) ─────────
// 각 스타일 설명에서 문자/타이포그래피 관련 표현 제거
// NO TEXT 제약 조건 강화
...
function buildPrompt(headline, bodyText, tags, context) {
  ...
  // 강화된 NO TEXT 제약: FLUX가 생성하는 깨진 텍스트 완전 차단
  const noTextRule = 'CRITICAL: GENERATE ZERO TEXT. Absolutely no letters, no characters, no words, no numbers, no labels, no captions, no watermark, no logo, no typography, no signage, no writing of any kind. No Korean characters, no English letters. NO TEXT WHATSOEVER. Pure visual imagery only. If the model tries to add text, it will produce garbled broken characters which is unacceptable. Pure photography or illustration without any textual elements.';
...
function saveImageFromResult(result) {
  ...
  // FLUX 2 Turbo는 URL 반환 (data: URI가 아님)
  if (!imgUrl || imgUrl.startsWith('data:')) throw new Error('Unexpected data URI from Turbo: ' + (imgUrl || 'undefined'));
  const ext = imgUrl.match(/\.(png|jpg|jpeg|webp)(\?|$)/i)?.[1] || 'png';
  const localPath = `/tmp/flux-article-${Date.now()}.${ext}`;
  return new Promise((resolve, reject) => {
...
if (require.main === module) {
  ...
  if (!headline) { console.error('사용법: node generate-article-image.js <headline> [bodyText] [tags]'); process.exit(1); }
  generateImageForArticle({ headline, bodyHtml: bodyText, tags })
    .then(r => console.log(`\n🎯 ${headline}\n   Style: ${r.style}\n   Image: ${r.url}`))
    .catch(err => { console.error(`\n❌ ${err.message}`); process.exit(1); });
}

module.exports = { generateImageForArticle, detectCountryContext, buildPrompt, selectStyle, saveImageFromResult };
